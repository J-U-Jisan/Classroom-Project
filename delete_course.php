<?php
	...
	if(isset($_POST['submit'])){
		...
		$courseno = $_POST['submit'];
		$admin_id = $_SESSION['userid'];

		$data = array('courseno' => $courseno, 'admin_id' => $admin_id);
		
		$response = Requests::post('http://127.0.0.1/apipro/courses/delete.php', array(), $data);
		$rt= $response->body;
		?>
		<div>
			<script>
		...
		foreach ($data as $key => $value){
		 	if($value['courseno']==$courseno){
		 		$data = array('courseno' => $courseno, 'studentid' => $value['studentid'], 'admin_id' => $admin_id);		
				$response = Requests::post('http://127.0.0.1/apipro/assign_student/delete.php', array(), $data);
			//	var_dump($response->body);
		 	}
		}
...
<body style="background-color: #858ea1;">
	...
	<header>
		<nav>
		    ...
		    <div class="dropdown" style="margin-left: 10px;">
		    <button class="dropbtn" style="color: blue">Course 
		    <span style="transform: rotate(90deg);display: block;float:right;margin-left: 8px;">&#x27A7;</span>
		    </button>
		    <div class="dropdown-content">
			...
			<?php
				...
				foreach ($data as $key =>$value){
					if($value['admin_id']== $_SESSION['userid'] && $value['courseno']!='111'){
						?>
					<button id="submit"type="submit" name="submit" value="<?php echo $value['courseno'];?>" style="width: 61%; margin-left: 20%;height: 45px;font-size: 20px;margin-top: 10px;" onclick="return ConfirmDelete();">
						<div style="width: 30%;background-color:#0cf75d;margin-left: -33%;display: inline-block;position: relative; top: -1px; padding: 9px;padding-bottom: 8px;"><?php echo $value['courseno'];?></div><div style="width: 66%;background-color:#1ce829;margin: 0 auto;display: inline-block;margin-right: -33%;padding: 9px;padding-bottom: 8px;position: relative;top: -1px;padding-right: 10px;"><?php echo $value['coursetitle']; ?></div>
